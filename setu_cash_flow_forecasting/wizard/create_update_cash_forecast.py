...
class CreateUpdateCashForecast(models.TransientModel):
    ...
    calculated_base_on = fields.Selection(selection=[('real', 'Real Account Data'),
                                                     ('demo', 'Imported Account Data')],
                                          string='Calculate Base on')
    ...
    def prepare_create_manual_cash_forecast(self, cash_forecast_type_id, period_id):
        forecast_value = cash_forecast_type_id.with_context(calculated_base_on=self.calculated_base_on
                                                            )._get_forecast_value(period_id) or 0.0
        return {
            ...
            'analytic_account_ids': [(6,0,cash_forecast_type_id.analytic_account_ids.ids)],
        ...
        if not cash_forecast_type_ids:
            cash_forecast_type_ids = self.env['setu.cash.forecast.type'].search(
                                            [('company_id', '=', period_id.company_id.id),
                                             '|', ('calculate_from', 'in', ['pending','uninvoiced_sales','unbilled_purchase']),
    ...
    def calculate_value(self):
        ...
        # self.company_id.set_onboarding_step_done('create_forecast_data_state')
        self.env['onboarding.onboarding.step'].sudo().action_validate_step(
            'setu_cash_flow_forecasting.onboarding_onboarding_step_create_forecast')
        return {
