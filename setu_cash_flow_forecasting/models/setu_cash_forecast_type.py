...
class SetuCashForecastType(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not self.env.context.get('demo_data', False) and vals.get('type') in ['income', 'expense', 'opening'] and \
        ...
        return super(SetuCashForecastType, self).create(vals_list)

    def write(self, vals):
        res = super(SetuCashForecastType, self).write(vals)
        for rec in self:
            if not self.env.context.get('demo_data', False) and \
    ...
    def _get_opening_balance(self, forecast_period_id):
        ...
        if previous_cash_forecast := self.env['setu.cash.forecast'].search(
            ...
            if self.env.context.get('calculated_base_on', False) != 'demo' else 'local_account_book aml'}
                where
                    {"am.state = 'posted' and" if self.env.context.get('calculated_base_on', False) != 'demo' else ''}
            ...
            balance = self.env.cr.fetchall()[0][0]
        return balance or 0
    ...
    def _get_past_account_entries_forecast_value(self, forecast_period_id):
        ...
        if self.env.context.get('calculated_base_on', False) == 'demo':
    ...
    def _get_unbilled_purchase_value(self, forecast_period_id):
        ...
        self.env.cr.execute(query)
        unbilled_purchase_data = self.env.cr.dictfetchall()
    ...
    def _get_uninvoiced_sales_value(self, forecast_period_id):
        ...
        self.env.cr.execute(query)
        uninvoiced_sale_data = self.env.cr.dictfetchall()
