...
class CashForecastFiscalYear(models.Model):
    ...
    def unlink(self):
        if self.env['setu.cash.forecast'].search([('forecast_period_id', 'in', self.fiscal_period_ids.ids)]):
    ...
    def document_layout_save(self):
        # self.company_id.set_onboarding_step_done('fiscal_year_setup_data_state')
        return self.env['onboarding.onboarding.step'].action_validate_step(
            'setu_cash_flow_forecasting.onboarding_onboarding_step_fiscal_year')
