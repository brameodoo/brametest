...
class TallerSolicitud(models.Model):
    ...
    def action_ingresar_taller(self):
        for record in self:
            # 1. Validación de estado: Asegura que solo se avance desde 'assigned'
            if record.state != 'assigned':
                raise UserError("Solo puedes marcar como 'Ingreso a Taller' solicitudes con fecha asignada.")
            
            # 2. **NUEVA VALIDACIÓN:** Asegura que la fecha de ingreso a taller esté llena
            if not record.fecha_ingreso_taller:
                raise UserError("Debes especificar la 'Fecha de Ingreso a Taller' antes de avanzar.")

            # Si ambas validaciones pasan, entonces se cambia el estado
            record.state = 'in_workshop'
            # La línea record.fecha_ingreso_taller = fields.Datetime.now() está comentada
            # en tu código original, lo cual es correcto si el usuario debe establecerla manualmente.
            # Si quisieras que se llene automáticamente al presionar el botón, deberías descomentarla
            # y, si ya tiene valor, decidir si quieres permitir que se sobrescriba o no.
            # Por la descripción del problema, parece que la quieres manual, así que la mantenemos comentada.

    def action_reparado(self):
        for record in self:
            # **NUEVA VALIDACIÓN:** Asegura que la fecha de Reparación Terminada esté llena
            if not record.fecha_reparacion_terminada:
                raise UserError("Debes especificar la 'Fecha de Reparación Terminada' antes de marcar como reparado.")
            ...
            # La fecha se asigna aquí, lo cual es coherente si el usuario no la pone manualmente antes
            # pero el botón la establece al momento de ser presionado.
            # Si el usuario *debe* ponerla manualmente, entonces esta línea se debería eliminar
            # y añadir una validación similar a la de fecha_ingreso_taller.
            # Por la consistencia con tu código, la mantendremos.
